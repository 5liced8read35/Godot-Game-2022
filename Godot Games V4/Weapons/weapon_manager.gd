...
func _ready():
	...
	get_parent().get_node("/root/World/NewFPS/Head/Camera/AimCast").add_exception(owner) # Adds exception of player to the shooting raycast
	
	all_weapons = {
		"Unarmed" : preload("res://Weapons/unarmed/unarmed.tscn"),
		"Pistol_A" : preload("res://Weapons/armed/pistol/pistol_a.tscn"),
		"Rifle_A" : preload("res://Weapons/armed/rifle/rifle_a.tscn"),
	}
	...
	for w in weapons:
		if weapons[w] != null:
			...
			weapons[w].ray = get_parent().get_node("/root/World/NewFPS/Head/Camera/AimCast")
